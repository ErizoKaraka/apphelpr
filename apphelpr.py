...
def main (page:Page):
    ...
    def on_message(client, userdata, msg):
            ...
            if msg.topic == "datos":
                ...
                lista_datos.pop(0)
                lista_datos.pop(1)
                data_string = lista_datos[0]
                print(data_string)
                data_dict = json.loads(data_string)
                print(data_dict)
                pulsi = str(data_dict["pulso"])
                osi = str(data_dict["oxigeno"])
                tepe = str(data_dict["temperatura"])
                fall = str(data_dict["caida"])
            elif msg.topic == "logueo2":
                print(msg.topic)
                log = str(msg.payload)
                lista_logueo = log.split("'")
                lista_logueo.pop(0)
                lista_logueo.pop(1)
                ans = lista_logueo[0].split(".")[0]
                if ans == "1":
                    usuario = f"{lista_logueo[0].split(".")[1]}"
                    print("vamos")
                    page.go("/mediciones")
                elif ans == "0":
                    no_user.visible = True
                    page.update()
                elif ans == "2":
                    no_user.visible = True
                    page.update()
    ...
    def on_disconnect(client, userdata, reason,  rc):
    ...
    client.username_pw_set("EnzoDubini", "Papa1234")
    client.connect("603c0cdebb534ca3ad7a66a57e8651c5.s1.eu.hivemq.cloud", 8883)
    client.subscribe("contactos", qos=0)
    client.subscribe("datos", qos=0)
    client.subscribe("alarma", qos=1)
    client.subscribe("logueo", qos=1)
    client.subscribe("logueo2", qos=1)
    ...
    page.fonts = {
        "phil": "./fonts/ChauPhilomeneOne-Regular.ttf"
    }
    ...
    perfil = Container(
        ...
        width=20,
        height=20,
        content= ft.Image(src="./images/contacto.jpg"),
        ...
        async def update_cai(self):
            ...
            while self.running:
                if int(fall) == 1:
                    self.visible = True
                    self.update()
                    client.publish("alarma", payload=1, qos=1)
                elif int(fall) == 0:
                    self.visible = False
                    self.update()
                self.update()
    ...
    class num_pul(ft.Text):
        ...
        async def update_pul(self):
            ...
            while self.running:
                self.value = f"{pulsi}"
                if int(pulsi) < int(pulsi_n1) or int(pulsi) > int(pulsi_n2):
                    client.publish("alarma", payload=3, qos=1)
                self.update()
                await asyncio.sleep(10)
    ...
    class num_ox(ft.Text):
        ...
        async def update_ox(self):
            ...
            while self.running:
                self.value = f"{osi}"
                self.update()
                if int(osi) < int(osi_n1) or int(osi) > int(osi_n2):
                    client.publish("alarma", payload=4, qos=1)
                await asyncio.sleep(10)
    ...
    class num_temp(ft.Text):
        ...
        async def update_temp(self):
            ...
            while self.running:
                self.value = f"{tepe}"
                if int(tepe) < int(tepe_n1) or int(tepe) > int(tepe_n2):
                    client.publish("alarma", payload=2, qos=1)
                self.update()
                await asyncio.sleep(10)
            ...
            page.views.append(
                View(
                    ...
                    [
                        ...
                        wrap = False,
                        controls=[
                            Text(f"{usuario}", size=40, font_family="phil"), perfil
                        ]
                    ),
                    ft.Divider(
                        height=20,
                            ...
                            col_p(
                                [   
                                    Row(
                                        wrap = False,
                                        controls=[
                                            Text(f"{usuario}", size=40, font_family="phil")
                                        ]
                                    ),
                                    Divider(
                                        color="#6C6A73"
    ...
    def añadcontacto(e):
        ...
        client.publish("contactos", payload=f"{lista_contactos[-1]}", qos=0)
    ...
    def mediciones(e):
        client.publish("logueo", payload=f"{nombre.content.value}.{contraseña.content.value}", qos=1)
        time.sleep(10)
